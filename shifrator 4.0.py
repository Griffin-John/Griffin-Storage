...
def zashifr(text, items, flag=True): 
    global bolshbukvi
    shifr = ''
    for letter in range(len(text)):
        for j in items:
            if text[letter].lower() == j[1]:
                kod = ''
                a = j[0]
...
def clicked_zashifr():
    ...
    nseed = str(random.randint(100000, 999999))
    text = zashifr(txt_zashifr.get(1.0, tk.END), seed(nseed), True)
...
def clicked_razshifr():
    ...
    text = zashifr(root.clipboard_get().split()[1], seed(nseed*10), False)
